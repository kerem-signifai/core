...
public final class BMCommandHandler implements Listener {
    ...
    public static void init() {
        ...
        CorePlugin.getInstance().getServer().getPluginManager().registerEvents(new BMCommandHandler(), CorePlugin.getInstance());
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                try {
                    ...
                    Field commandMapField = CorePlugin.getInstance().getServer().getClass().getDeclaredField("commandMap");
                    commandMapField.setAccessible(true);

                    Object oldCommandMap = commandMapField.get(CorePlugin.getInstance().getServer());
                    BMCommandMap newCommandMap = new BMCommandMap(CorePlugin.getInstance().getServer());
        ...
        BMCommandHandler.loadCommandsFromPackage(CorePlugin.getInstance(), "com.keremc.core.command.commands");
    ...
    protected static void registerMethod(Method method) {
        ...
        for (int parameterIndex = 1; parameterIndex < method.getParameterTypes().length; parameterIndex++) {
            ...
            if (parameterAnnotation != null) {
                ...
                CorePlugin.getInstance().getLogger().warning("Method '" + method.getName() + "' has a parameter without a @Parameter annotation.");
                return;
            }
        }
    ...
    public static String[] getParameters(Player player) {
        return BMCommandMap.parameters.get(player.getUniqueId());
    }
    ...
    public void onCommandPreProcess(PlayerCommandPreprocessEvent event) {
        ...
        BMCommandMap.parameters.put(event.getPlayer().getUniqueId(), command.split(" "));
