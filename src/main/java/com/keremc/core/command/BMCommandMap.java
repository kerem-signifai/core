...
final class BMCommandMap extends SimpleCommandMap {

    protected static Map<UUID, String[]> parameters = new HashMap<>();

    public BMCommandMap(Server server) {
        super(server);
    }
    ...
    public List<String> tabComplete(CommandSender sender, String cmdLine) {
        ...
        try {
            ...
            CommandLoop:
            for (CommandData command : BMCommandHandler.getCommands()) {
                ...
                for (String alias : command.getNames()) {
                    ...
                    if (StringUtil.startsWithIgnoreCase(split.trim(), cmdLine.trim()) || StringUtil.startsWithIgnoreCase(cmdLine.trim(), split.trim())) {
                        if (spaceIndex == -1 && cmdLine.length() < alias.length()) {
                            ...
                            for (String completion : BMCommandHandler.tabCompleteParameter(player, cmdLine.endsWith(" ") ? "" : params[params.length - 1], paramData.getParameterClass(), paramData.getTabCompleteFlags())) {
